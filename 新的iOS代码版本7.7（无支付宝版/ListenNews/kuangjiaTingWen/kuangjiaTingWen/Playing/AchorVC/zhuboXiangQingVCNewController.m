...
//                break;
    ...
    pagingView.clickEventViewsBlock = ^(UIView *eventView) {
        ...
        }else if ([eventView isKindOfClass:[UILabel class]]){
            UILabel *label = (UILabel *)eventView;
            if ([label.accessibilityLabel isEqualToString:@"名称"]) {
                [self nameTap];
            }
        }
...
- (void)nameTap
{
    ...
    vc.jiemuDescription = self.jiemuDescription;
    vc.jiemuFan_num = self.jiemuFan_num;
    vc.jiemuID = self.jiemuID;
    vc.jiemuImages = self.jiemuImages;
    vc.jiemuIs_fan = self.jiemuIs_fan;
    vc.jiemuMessage_num = self.jiemuMessage_num;
    vc.jiemuName = self.jiemuName;
    vc.act_id = self.jiemuID;
    vc.listVC = self;
    vc.hidePayBtn = YES;
