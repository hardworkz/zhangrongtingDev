...
//            detailNews.backgroundColor = [UIColor redColor];
        ...
        [downloadBtn setFrame:CGRectMake(CGRectGetMaxX(dataLab.frame), 81.0 / 667 *IPHONE_H + offsetY, 35.0 / 667 *IPHONE_H, 35.0 / 667 *IPHONE_H)];
        if (IS_IPHONEX){
            [downloadBtn setFrame:CGRectMake(CGRectGetMaxX(dataLab.frame), 81.0 + offsetY, 35.0, 35.0)];
        }
...
{
    _dataDict = dataDict;
//    RTLog(@"post_mp----------%@",dataDict[@"post_mp"]);
    //图片
...
{
    if ([[ZRT_PlayerManager manager] limitPlayStatusWithPost_id:self.dataDict[@"id"] withAdd:NO]) {
