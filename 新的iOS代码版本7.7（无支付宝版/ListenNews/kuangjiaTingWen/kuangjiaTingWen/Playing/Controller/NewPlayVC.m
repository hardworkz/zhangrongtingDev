...
- (void)viewDidLoad {
    ...
    self.dateFont = [[CommonCode readFromUserD:DateFont] floatValue]?[[CommonCode readFromUserD:DateFont] floatValue]:16.0;
    ...
    if (!self.timer) {
        self.timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(viewTimeAction) userInfo:nil repeats:YES];
    }
...
- (void)setFrameModel{
    ...
    frameModel.timeString = self.isFormClass?[NSString stringWithFormat:@"%@ ",[date showTimeByTypeA]] : [NSString stringWithFormat:@"#来自:%@   %@ ",self.postDetailModel.post_lai,[date showTimeByTypeA]];
...
//    bofangdingshiBtn.backgroundColor = [UIColor redColor];
    ...
    dibuBtnView = [[UIView alloc]initWithFrame:CGRectMake(0,IPHONE_H - (IS_IPHONEX? 50 + 34:50) , IPHONE_W, IS_IPHONEX? 50 + 34:50)];
    dibuBtnView.backgroundColor = HEXCOLOR(0xfafafa);
    [self.view insertSubview:dibuBtnView aboveSubview:dibuView];
    
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, 0, IPHONE_W, 0.5)];
    line.backgroundColor = [UIColor grayColor];
    line.alpha = 0.5f;
    [dibuBtnView addSubview:line];
    //功能按钮
    int col = 3;
    for (int i = 0; i<col; i++) {
...
- (UITableView *)tableView
{
    if (!_tableView)
    {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0,IS_IPHONEX?-25:0, IPHONE_W, IPHONE_H - (IS_IPHONEX ? 50 + 34 - 25:50)) style:UITableViewStylePlain];
        ...
        [_tableView addTapGesWithTarget:self action:@selector(tableViewTap)];
    }
    return _tableView;
}
- (void)tableViewTap
{
    if (dibuView.y == IPHONE_H - 159.0 / 667 * IPHONE_H) {
        [UIView animateWithDuration:0.5 animations:^{
            dibuView.y = IPHONE_H - (IS_IPHONEX ? 50 + 34:50);
        }];
    }else{
        [UIView animateWithDuration:0.5 animations:^{
            dibuView.y = IPHONE_H - 159.0 / 667 * IPHONE_H;
        }];
    }
}
...
{
    DefineWeakSelf
    [NetWorkTool zanNewsUploadWithAccessToken:AvatarAccessToken post_id:self.post_id times:[NSString stringWithFormat:@"%ld",(long)zanTouchCount] sccess:^(NSDictionary *responseObject) {
        if ([responseObject[status] intValue] == 1) {
...
- (void)viewTimeAction
{
    ...
    if (timeCount <= 0) {
        ...
        if (self.tableView.contentOffset.y < 209.0 / 667 * SCREEN_HEIGHT) {
            [UIView animateWithDuration:0.5 animations:^{
                dibuView.y = IPHONE_H - 159.0 / 667 * IPHONE_H;
            }];
        }
...
- (void)addTimer
{
    ...
    self.timer = nil;
    NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(viewTimeAction) userInfo:nil repeats:YES];
