...
- (void)getHomeNewsData
{
    DefineWeakSelf
    NSString *IDType = nil;
    [NetWorkTool getInformationNewListWithaccessToken:AvatarAccessToken andId:nil andType:IDType andLimit:nil andPage:@"1" sccess:^(NSDictionary *responseObject) {
        if ([responseObject[status] intValue] == 1){
            ...
            [weakSelf getPushNewsDetailWithArray:dictArray];
        }
...
/**
 根据推送ID获取新闻详情数据
 */
- (void)getPushNewsDetailWithArray:(NSMutableArray *)array
{
    ...
    [NetWorkTool getpostinfoWithpost_id:[CommonCode readFromUserD:pushNews] andpage:nil andlimit:nil sccess:^(NSDictionary *responseObject) {
        if ([responseObject[status] integerValue] == 1) {
            ...
            for (NSDictionary *dict in array) {
                if (dict[@"id"] == responseObject[results][@"id"]) {
                    isExist = YES;
                    break;
                }
            }
            if (!isExist) {
                [array insertObject:responseObject[results] atIndex:0];
            }
            ...
            [weakSelf presentPushNewsWithArray:array];
        }
        else{
    ...
    [CommonCode writeToUserD:nil andKey:pushNews];
}
/**
 通知消息点击跳转新闻详情界面播放
 */
- (void)presentPushNewsWithArray:(NSMutableArray *)array
{
    [ZRT_PlayerManager manager].songList = array;
