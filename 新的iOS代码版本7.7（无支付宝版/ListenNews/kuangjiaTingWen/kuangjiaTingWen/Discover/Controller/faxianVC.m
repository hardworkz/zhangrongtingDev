...
@property(strong,nonatomic)UITableView *SouSuoTextTableView;
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.SouSuoTextTableView];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    else if (tableView == self.SouSuoTextTableView){
...
//        [cell.contentView addSubview:faxianshutiao];
        ...
        faxianjiantou.frame = CGRectMake(IPHONE_W - 28.0 / 375 * IPHONE_W, faxiangengduoBtn.frame.origin.y + 2.0 / 667 * IPHONE_H, faxiangengduoBtn.frame.size.height - 5.0 / 667 * IPHONE_H, faxiangengduoBtn.frame.size.height - 5.0 / 667 * IPHONE_H);
        faxianjiantou.centerY = faxiangengduoBtn.centerY;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.SouSuoTextTableView){
        ...
        if (indexPath.row == 0){
            if (IS_IPAD) {
                ...
                return IS_IPHONEX?173.5 / 667 * 667: 173.5 / 667 * IPHONE_H;
            }
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.SouSuoTextTableView){
        ...
        [self.SouSuoTextTableView reloadData];
        
        self.SouSuoTextTableView.hidden = YES;
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar {
    ...
    self.SouSuoTextTableView.hidden = NO;
...
- (void)faxianliebiaoGengDuoAction:(UIButton *)sender{
    ...
    if (indexPath.row == 0) {
        ...
        faxianMoreViewController *faxiangengduoVC = [[faxianMoreViewController alloc]init];
        if ([model.type isEqualToString:@"主播"]) {
            faxiangengduoVC.isAchor = YES;
        }else{
            faxiangengduoVC.isAchor = NO;
        }
        faxiangengduoVC.titleText = model.type;
...
- (UITableView *)SouSuotableView {
    if (!_SouSuotableView){
        _SouSuotableView = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.SouSuosearchBar.frame), IPHONE_W ,IS_IPHONEX? IPHONE_H - CGRectGetMaxY(self.SouSuosearchBar.frame) - IPHONEX_BOTTOM_H:IPHONE_H - CGRectGetMaxY(self.SouSuosearchBar.frame) - 49) style:UITableViewStylePlain];
...
- (UITableView *)SouSuoTextTableView
{
    if (!_SouSuoTextTableView)
    {
        _SouSuoTextTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.SouSuosearchBar.frame), IPHONE_W,IS_IPHONEX? IPHONE_H - CGRectGetMaxY(self.SouSuosearchBar.frame) - IPHONEX_BOTTOM_H:IPHONE_H - CGRectGetMaxY(self.SouSuosearchBar.frame) - 49) style:UITableViewStyleGrouped];
        _SouSuoTextTableView.backgroundColor = [UIColor clearColor];
        _SouSuoTextTableView.delegate = self;
        _SouSuoTextTableView.dataSource = self;
        _SouSuoTextTableView.tag = 2;
        _SouSuoTextTableView.hidden = YES;
...
//        [cleanBtn addTarget:self action:@selector(cleanHistorySearchWord)];
        ...
        _SouSuoTextTableView.tableFooterView = [UIView new];
    }
    return _SouSuoTextTableView;
}
