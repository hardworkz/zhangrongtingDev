...
@property (strong, nonatomic) UIButton *activityMoneyBtn;
...
- (void)viewDidLoad {
    ...
    self.titleFontSize = [[CommonCode readFromUserD:TitleFontSize] floatValue]?[[CommonCode readFromUserD:TitleFontSize] floatValue]:24.0;
    self.contentFont = [[CommonCode readFromUserD:DateFont] floatValue]?[[CommonCode readFromUserD:DateFont] floatValue]:16.0;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (_hidePayBtn) {
        self.auditionnBtn.hidden = YES;
        self.purchaseBtn.hidden = YES;
        self.seperatorLine.hidden = YES;
        self.helpTableView.height = SCREEN_HEIGHT;
    }else{
        self.auditionnBtn.hidden = NO;
        self.purchaseBtn.hidden = NO;
        self.seperatorLine.hidden = NO;
        self.helpTableView.height = SCREEN_HEIGHT - 49;
    }
...
//    [self.navigationController setNavigationBarHidden:NO animated:YES];
    ...
    self.activityMoneyBtn.x = SCREEN_WIDTH - 30;
}
...
- (void)setupAnimationBtn
{
    ...
    shake.duration = 0.35;//执行时间
    shake.autoreverses = YES; //是否重复
    shake.repeatCount = 3;//次数
    [self.activityBtn.layer addAnimation:shake forKey:@"shakeAnimation"];
}
- (void)setUpView{
    ...
    self.seperatorLine = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.helpTableView.frame), SCREEN_WIDTH, 1.0)];
    ...
    [self.view insertSubview:self.activityBtn aboveSubview:self.helpTableView];
    [self.view insertSubview:self.activityMoneyBtn belowSubview:self.activityBtn];
}
...
- (NSMutableArray *)frameArrayWithClassModel:(ClassModel *)classModel
{
    ...
    //课堂内容frame
    self.titleLab.font = CUSTOM_FONT_TYPE_FZYANS(self.titleFontSize);
    ClassContentCellFrameModel *contentFrameModel = [ClassContentCellFrameModel new];
    contentFrameModel.titleFontSize = self.contentFont;
...
- (void)setTableHeadView{
    ...
    CGFloat titleHight = [self computeTextHeightWithString:self.classModel.title andWidth:(SCREEN_WIDTH-20) andFontSize:CUSTOM_FONT_TYPE_FZYANS(self.titleFontSize)];
    [_titleLab setFrame:CGRectMake(20.0 , CGRectGetMaxY(_zhengwenImg.frame) + 20.0, IPHONE_W - 40.0, (titleHight + 20))];
    
    [_xiangqingView addSubview:self.titleLab];
    
    _seperatorLine.frame = CGRectMake(20.0 / 375 * SCREEN_WIDTH, CGRectGetMaxY(_titleLab.frame) +  12.0, SCREEN_WIDTH - 40.0 / 375 * SCREEN_WIDTH, 1.0);
...
- (void)timeAction
{
//    RTLog(@"timeCount%ld",t   imeCount);
    timeCount--;
    if (timeCount <= 0) {
        [self removeTimer];
        
        [UIView animateWithDuration:0.5 animations:^{
            self.activityMoneyBtn.x = SCREEN_WIDTH - self.activityMoneyBtn.width;
        }];
    }
}
/**
...
- (void)activeClicked
{
    LearingInvitationCardController *learnVC = [[LearingInvitationCardController alloc] init];
    learnVC.view.backgroundColor = [UIColor whiteColor];
...
//    [alert show];
    [_alertView coverClick];
    UIAlertController *qingshuruyonghuming = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"您的听币不足，请先前往充值，当前听币余额为(0)" preferredStyle:UIAlertControllerStyleAlert];
    [qingshuruyonghuming addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    }]];
    [qingshuruyonghuming addAction:[UIAlertAction actionWithTitle:@"前往充值" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)auditionnBtnAction:(UIButton *)sender
{
    ...
    if ([ZRT_PlayerManager manager].isPlaying && sender.selected == YES) {//选中状态，为播放状态,则将列表按钮设置全部设置为暂停
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        cell.titleFontSize = self.contentFont;
...
- (UITableView *)helpTableView{
    if (_helpTableView == nil) {
        ...
        _helpTableView.estimatedRowHeight = 0;
        _helpTableView.estimatedSectionHeaderHeight = 0;
        _helpTableView.estimatedSectionFooterHeight = 0;
...
{
    if (!_activityBtn) {
        _activityBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 26, 300, 20, 20)];
        [_activityBtn setImage:@"icon_money"];
        _activityBtn.layer.borderWidth = 0.5;
...
- (UIButton *)activityMoneyBtn
{
    if (!_activityMoneyBtn) {
        _activityMoneyBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 30, _activityBtn.y - 5, 100, 30)];
        _activityMoneyBtn.backgroundColor = ColorWithRGBA(251, 168, 52, 1);
        _activityMoneyBtn.layer.cornerRadius = 15;
        _activityMoneyBtn.layer.borderColor = [UIColor whiteColor].CGColor;
...
- (UILabel *)titleLab
{
    if (_titleLab == nil) {
        //课堂标题
        _titleLab = [[UILabel alloc] initWithFrame:CGRectMake(20.0 / 375 * IPHONE_W,CGRectGetMaxY(_zhengwenImg.frame) + 20.0 , IPHONE_W - 40.0 / 375 * IPHONE_W, 40.0 / 667 * IPHONE_H)];
        _titleLab.textAlignment = NSTextAlignmentCenter;
        _titleLab.textColor = nTextColorMain;
        _titleLab.font = CUSTOM_FONT_TYPE_FZYANS(self.titleFontSize);
...
- (void)vipSelected:(UIButton *)button
{
    ...
    vipPriceLabel.text = [VipSelected.accessibilityIdentifier isEqualToString:@"vip"]?_classModel.svipPrice?[NSString stringWithFormat:@"¥%@",_classModel.svipPrice]:@"¥0":_priceLabel.text;
}
- (void)classSelected
{
    RTLog(@"classSelected");
