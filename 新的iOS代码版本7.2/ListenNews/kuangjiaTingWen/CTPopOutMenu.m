...
//            self.titleLabel.backgroundColor = [UIColor clearColor];
        ...
        if (item.image!=nil) {
            self.image = [item.image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
...
-(void)layoutMenuItemViewasDefault{
    ...
    if (self.iconIamageView!= nil) {
        ...
        self.iconIamageView.frame = CGRectMake(self.bounds.size.width*0.05 , self.bounds.size.height*0.05, 3 *imageSize.width / 4, 3 * imageSize.height / 4);
    }else{
    ...
    if (self.iconIamageView == nil) {
        self.titleLabel.center = CGPointMake(self.bounds.size.width/2  - 30, self.bounds.size.height/2);
    }
    if (self.titleLabel == nil) {
...
-(void)layoutMenuItemViewasOval{
    ...
    CGSize imageSize;
    if (self.iconIamageView!=nil) {
        imageSize = IS_IPAD ? CGSizeMake(self.bounds.size.width*0.4, self.bounds.size.height-labelSize.height-20) : CGSizeMake(self.bounds.size.width*0.7, self.bounds.size.height-labelSize.height- 5);
        self.iconIamageView.frame = !IS_IPAD ? CGRectMake(self.bounds.size.width*0.15 + 15, self.bounds.size.height*0.05, imageSize.width / 2, imageSize.height) : CGRectMake(self.bounds.size.width*0.15 + 15 , self.bounds.size.height*0.05 - 10, 3 * imageSize.width / 5, imageSize.height);
//        self.iconIamageView.contentMode = UIViewContentModeScaleAspectFit;
        self.titleLabel.frame = !IS_IPAD ? CGRectMake(self.bounds.size.width*0.05, CGRectGetMaxY(self.iconIamageView.frame), labelSize.width, labelSize.height) : CGRectMake(self.bounds.size.width*0.05 - 30, CGRectGetMaxY(self.iconIamageView.frame) + 10, labelSize.width, labelSize.height);
    }else{
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message items:(NSArray *)items{
    if (self = [super init]) {
        _items = items;
        _titleText = title;
        self.view.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.37];
...
-(void)showMenuInParentViewController:(UIViewController *)parentVC withCenter:(CGPoint)center{
    ...
    self.menuCenter = CGPointMake(center.x, center.y - 100);
...
-(void)dismissMenu{
    [UIView animateWithDuration:0.2 delay:0 usingSpringWithDamping:0.5 initialSpringVelocity:0.75 options:0 animations:^{
        if (self.menuView.superview!=nil) {
            self.menuView.transform = CGAffineTransformMakeScale(0.0, 0.0);
        }
...
//    self.menuView.layer.masksToBounds = YES;
    ...
    [self.titleView setTextColor:[UIColor blackColor]];
    ...
    [UIView animateWithDuration:0.2 delay:0 usingSpringWithDamping:1 initialSpringVelocity:0.75 options:0 animations:^{
...
-(void)layoutasGrid{
    ...
    [self.items enumerateObjectsUsingBlock:^(CTPopoutMenuItem * obj, NSUInteger idx, BOOL *stop) {
        ...
        if (idx >= [self.items count]-([self.items count]%colCount)) {
            ...
            CGFloat offSetX = (menuWidth - rowLength)/2;
            itemView = [[CTPopoutMenuItemView alloc]initWithMenuItem:obj
                                                               frame:CGRectMake(menuWidth *2 / 3 , 50,
                                                                                menuWidth / 3, 80)
                                                           menuStyle:self.menuStyle];
        }else{
            itemView = [[CTPopoutMenuItemView alloc]initWithMenuItem:obj
                                                               frame:CGRectMake(index*menuWidth/ 3 , 50,
                                                                                menuWidth / 3, 80)
                                                           menuStyle:self.menuStyle];
        }
        [self.menuView addSubview:itemView];
...
-(void)layoutTitleandMessagewithMenuWidth:(CGFloat)menuWidth{
    ...
    self.titleView.frame =  !IS_IPAD ? CGRectMake(menuWidth*0.05, FRAME_OFFSET, titleSize.width, titleSize.height) : CGRectMake(menuWidth*0.05 - 15, FRAME_OFFSET - 5, titleSize.width, titleSize.height);
...
-(CGRect)menuFramewithWidth:(CGFloat)width Height:(CGFloat)height Center:(CGPoint)center{
    ...
    CGRect frame = ! IS_IPAD ? CGRectMake(originX,originY,width,height) : CGRectMake(originX + 35,originY,width - 70,height - 25);
    return frame;
}
