...
- (void)setModel:(PlayVCCommentModel *)model
{
    ...
    if ([model.to_user_login length]) {//判断是否是回复内容
        ...
        CGSize contentSize = [[NSString stringWithFormat:@"%@ : %@",nbspStr,model.content] boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - _pinglunLabF.origin.x - 10, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16.0f]} context:nil].size;
        ...
        CGSize contentSize = [model.content boundingRectWithSize:CGSizeMake((SCREEN_WIDTH - CGRectGetMaxX(_pinglunImgF)/ 375 * IPHONE_W - 10), MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16.0f]} context:nil].size;
