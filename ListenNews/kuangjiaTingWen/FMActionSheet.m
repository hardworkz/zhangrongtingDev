    ...
    if (self = [super init]) {
        ...
        if (buttonTitles.count) {
            for (int i = 0; i < buttonTitles.count; i++) {
                ...
                NSTextAttachment *attachmenImage = [[NSTextAttachment alloc]init];
                if(i == 0){
                    attachmenImage.image = [UIImage imageNamed:@"user-7"];
                }else {
                    attachmenImage.image = [UIImage imageNamed:@"user-6"];
                }
                attachmenImage.bounds = CGRectMake(0, 0, 16, 16);
                NSMutableAttributedString *attbutedString = [[NSMutableAttributedString alloc]init];
                [attbutedString appendAttributedString:[NSAttributedString attributedStringWithAttachment:attachmenImage]];
                [attbutedString appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"   %@", buttonTitles[i]]]];
                
                [btn setAttributedTitle:attbutedString forState:UIControlStateNormal];
        ...
        [cancelBtn setTag:buttonTitles.count + 100];
...
- (void)didClickBtn:(UIButton *)btn {
    ...
    if ([_delegate respondsToSelector:@selector(actionSheets:clickedButtonAtIndex:)]) {
        [_delegate actionSheets:self clickedButtonAtIndex:btn.tag - 100];
    }
}
