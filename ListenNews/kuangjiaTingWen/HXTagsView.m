...
- (void)longPressButtonAction:(HXTagView *)tagView{
    
    for (HXTagView *view in self.subviews) {
        if ([view isKindOfClass:[HXTagView class]]) {
            UIButton *delete = [UIButton buttonWithType:UIButtonTypeCustom];
            [delete setFrame:CGRectMake(view.frame.size.width -20, 0, 20, 20)];
            [delete.layer setMasksToBounds:YES];
            [delete.layer setCornerRadius:10.0];
            [delete setBackgroundImage:[UIImage imageNamed:@"delete_on"] forState:UIControlStateNormal];
//            [delete setBackgroundColor:[UIColor redColor]];
            [delete setTag:view.tag];
            [delete addTarget:self action:@selector(delegateTagAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)delegateTagAction:(UIButton *)sender{
    
    NSMutableArray *tempMuary = [NSMutableArray new];
    tempMuary =  [_tagAry mutableCopy];
    [tempMuary removeObjectAtIndex:sender.tag];
    _tagAry = tempMuary;
    [self reloadData];
...
- (void)reloadData {
    ...
    for (int i = 0; i < disposeAry.count; i++) {
        ...
        for (int j = 0; j < iTags.count; j++) {
            ...
            tagButton.longPressButtonAction = ^(HXTagView *tagView) {
                [self longPressButtonAction:tagView];
            };
